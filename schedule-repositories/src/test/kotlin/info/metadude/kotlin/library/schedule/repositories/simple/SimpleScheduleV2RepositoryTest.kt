...
import info.metadude.kotlin.library.schedule.repositories.models.GetScheduleV2State.Error
import info.metadude.kotlin.library.schedule.repositories.models.GetScheduleV2State.Failure
import info.metadude.kotlin.library.schedule.repositories.models.GetScheduleV2State.Success
...
internal class SimpleScheduleV2RepositoryTest {

    private companion object {
        const val VALID_URL = "https://example.com/schedule2.json"
    ...
    @Test
    fun `getScheduleV2State returns success with schedule wrapper`() = runTest {
        api = mock {
        ...
        repository.getScheduleV2State(VALID_URL, requestETag = "", lastModifiedAt = "").test {
            val state = awaitItem()
            assertThat(state).isInstanceOf(Success::class.java)
            val success = state as Success
            assertThat(success.scheduleV2.schedule.conference.acronym).isEqualTo("test")
            assertThat(success.scheduleV2.schedule.events).hasSize(1)
            awaitComplete()
        }
    }

    @Test
    fun `getScheduleV2State returns error with http error`() = runTest {
        api = mock {
            on { provideScheduleService(any(), any(), any()) }
    ...
    @Test
    fun `getScheduleV2State returns failure with runtime exception`() = runTest {
        api = mock {
            on { provideScheduleService(any(), any(), any()) }
    ...
    @Test
    fun `getScheduleV2State returns failure with IllegalArgumentException`() = runTest {
        api = mock {
            on { provideScheduleService(any(), any(), any()) }
                .doReturn(ImmediatelySucceedingService())
        }
        ...
        repository.getScheduleV2State(INVALID_URL, requestETag = "", lastModifiedAt = "").test {
            val state = awaitItem()
            assertThat(state).isInstanceOf(Failure::class.java)
